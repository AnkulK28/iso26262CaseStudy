function varargout = checkCodeStdsWord(modelName, varargin)
%checkCodeStdsWord Check generated code against code standards
%   Check the generated code against code standards, and then generate the
%   Bug Finder report (MS Word).
%
%   checkCodeStdsWord(ModelName)
%   checkCodeStdsWord(ModelName, 'TreatAsTopMdl')
%   checkCodeStdsWord(ModelName, 'TreatAsTopMdl', 'CI')
%   checkCodeStdsWord(ModelName, [], 'CI')
...
psprjDir = fileparts(ProjArtifacts.getUnitCStdChksSummaryWord(modelName));
...
polyspace_report( '-template', tmplFiles, '-format', 'WORD', '-output-name', psprjDir, '-results-dir', resultDir, '-noview');

% rename Summary Report
tmpRpt1 = fullfile(psprjDir, 'PolyspaceProject_BugFinderSummary.docx');
rptFile = ProjArtifacts.getUnitCStdChksSummaryWord(modelName);
...
tmpRpt2 = fullfile(psprjDir, 'PolyspaceProject_CodingStandards.docx');
rptFile = ProjArtifacts.getUnitCStdChksWord(modelName);
