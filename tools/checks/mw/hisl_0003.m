...
% Compile the model.
interface = get_param(bdroot(system), 'Object');
init(interface, 'COMMAND_LINE', 'UpdateBDOnly', 'on');

blockTypes = { 'Sqrt' };

% Find all Sqrt blocks.
sqrtBlks = find_system(system,'MatchFilter',@Simulink.match.activeVariants, 'FollowLinks', 'on', 'LookUnderMasks', 'all', 'BlockType', blockTypes{1}, 'Operator', 'sqrt');
cBlks = {};
blks = {};
...
if ~isempty(sqrtBlks)
    ...
    for j = 1:length(sqrtBlks)
        if strcmpi(get_param(sqrtBlks{j}, 'OutputSignalType'), 'complex')
            cBlks{k} = sqrtBlks{j}; %#ok<AGROW>
            k = k + 1;
        elseif strcmpi(get_param(sqrtBlks{j}, 'OutputSignalType'), 'auto')
...
if ~isempty(blks)
    if license('test', 'fixed_point_toolbox')
        ...
        if strcmpi(get_param(system, 'Type'), 'block') && strcmpi(get_param(system, 'BlockType'), 'SubSystem')
            sys = bdroot(system);
        else
            sys = system;
        end
        ...
        % Note that converter.deriveMinMax() automatically terminates the
        % model when it returns.
        ...
        for j = 1:length(blks)
            ...
            result = converter.results(converter.CurrentRunName,@(r) (strcmpi(r.ResultName, [srcBlockObj.Path,'/',srcBlockObj.Name])));
            ...
            if isempty(result)
                result = converter.results(converter.CurrentRunName,@(r) (strcmpi(r.ResultName, [srcBlockObj.Path,'/',srcBlockObj.Name,' : Output'])));
            end
...
elseif ~hasFixedPointLicense
    modelAdvisorObject.setCheckResultStatus(false);
    ResultDescription{end+1} = ModelAdvisor.Text('License of Fixed-Point Designer is not available.');
else  
    % No violation.
